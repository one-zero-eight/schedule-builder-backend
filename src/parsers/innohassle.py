...
class InNoHassleEventsClient:
    ...
    async def get_event_groups(self) -> list[ViewEventGroup]:
        async with self.session() as s:
            async with s.get(f"{self.api_url}/event-groups/") as response:
                ...
                return [
                    ViewEventGroup(**group)
                    for group in groups_dict["event_groups"]
                ]
    ...
    async def batch_create_or_read_event_groups(
        ...
        async with self.session() as s:
            async with s.post(
                f"{self.api_url}/event-groups/batch-create-or-read", json=data
            ) as response:
                response.raise_for_status()
                groups_dict = await response.json()
                return [
    ...
    async def update_ics(self, event_group_id: int, ics_content: bytes) -> int:
        ...
        async with self.session() as s:
            async with s.put(
                ...
                raise Exception(
                    f"Unexpected response status: {response.status} for event group {event_group_id}"
                )
...
async def update_inh_event_groups(
    ...
    inh_event_groups = await inh_client.batch_create_or_read_event_groups(
        output.event_groups
    )
    ...
    tasks = [
        asyncio.create_task(task(event_group))

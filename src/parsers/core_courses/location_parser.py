...
def parse_location_string(x: str, from_parent: bool = False) -> Item | None:
    ...
    def get_location(y: str):
        ...
        if m := re.fullmatch(
            r"^((\d|ONLINE|ОНЛАЙН)+(?:\s*/\s*(\d|ONLINE|ОНЛАЙН)+)+)$", y
        ):
            locations = m.group(1)
    ...
    _starts_from_pattern = (
        r"\(?(STARTS ON|STARTS FROM|FROM|С)\s*(\d{1,2}[\/.]\d{1,2})\)?"
    )
    ...
    _starts_at_pattern = (
        r"\(?(STARTS|STARTS AT|НАЧАЛО В)\s*(\d{1,2}[:.]\d{1,2})\)?"
    )
    ...
    _two_modifiers_pattern = (
        rf"\(?(?P<first>{_mod_noname})\)?\s*\(?(?P<second>{_mod_noname})\)?"
    )
    ...
    def two_modifiers(y: str):
        if m := re.fullmatch(_two_modifiers_pattern, y):
            ...
            if as_z1 and as_z2:
                combined = as_z1.dict(exclude_none=True) | as_z2.dict(
                    exclude_none=True
                )
    ...
    if m := re.fullmatch(
        location_plus_pattern("two_modifiers", _two_modifiers_pattern), x
    ):
        location = get_location(m.group("location"))
        as_two_modifiers = two_modifiers(m.group("two_modifiers"))
        as_two_modifiers.location = location
        return as_two_modifiers
    _three_modifiers_pattern = rf"\(?(?P<first>{_mod_noname})\)?\s*\(?(?P<second>{_mod_noname})\)?\s*\(?(?P<third>{_mod_noname})\)?"
    ...
    def three_modifiers(y: str):
        if m := re.fullmatch(_three_modifiers_pattern, y):
            ...
            if as_z1 and as_z2 and as_z3:
                combined = (
                    as_z1.dict(exclude_none=True)
                    | as_z2.dict(exclude_none=True)
                    | as_z3.dict(exclude_none=True)
                )
                return Item.parse_obj(combined)
    ...
    if m := re.fullmatch(
        location_plus_pattern("three_modifiers", _three_modifiers_pattern), x
    ):
        location = get_location(m.group("location"))
    ...
    def _1(y: str):
        if m := re.fullmatch(__1, y):
            ...
            another = parse_location_string(
                m.group("another"), from_parent=True
            )
